...
class DDPM(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if stems_to_inpaint is not None:
            ...
            waveform, mix, z, target_waveform, target_mix= self.generate_sample(
                [batch],
                name=name,
                unconditional_guidance_scale=self.evaluation_params[
                    "unconditional_guidance_scale"
        ...
        waveforms = self.all_gather(waveform)
        mixs = self.all_gather(mix)
        zs= self.all_gather(z)
        target_waveforms = self.all_gather(target_waveform)
        target_mix= self.all_gather(target_mix)

        if self.trainer.world_size > 1 and self.global_rank == 0:
            for i in range(waveforms.shape[0]):
                self.save_all_audio(waveforms[i], mixs[i], zs[i], target_waveforms[i], target_mix[i])
        ...
        self.log_dict(
            loss_dict_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True, sync_dist=True
        )
    ...
    def generate_sample(
        ...
        **kwargs,
        ...
        with self.ema_scope("Plotting"):
            ...
            mix = waveform_reshaped.sum(dim=1)

        return waveform.detach(), mix.detach(), z.detach(), target_waveforms.detach(), target_mix.detach()
    
    def save_all_audio(self, waveform, mix, z, target_waveform, target_mix, name="waveform", **kwargs):
        ...
        target_waveforms = target_waveform.detach().cpu().numpy()
        target_mix = target_mix.detach().cpu().numpy()
        fnames = [fake.pystr() for i in range(waveform.shape[0])]

        waveform_save_path = os.path.join(self.get_log_dir(), "val_%s" % (self.global_step))
        os.makedirs(waveform_save_path, exist_ok=True)
        ...
        self.save_waveform(target_mix[:,np.newaxis,:], target_mix_dir, name=fnames)
        ...
        for i in range(self.num_stems):
            ...
            self.save_waveform(target_waveforms[:,i,:][:,np.newaxis,: ], target_stem_dir, name=fnames)
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        assert self.conditioning_key in [
            None,
            "concat",
            "crossattn", # geowizard's condition mixing method
            "hybrid",
            "adm",
            "film",
        ]
