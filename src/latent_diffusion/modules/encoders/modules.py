...
class CLAPAudioEmbeddingClassifierFreev2(nn.Module):
    ...
    def cos_similarity(self, waveform, text):
        ...
        with torch.no_grad():
            ...
            audio_emb = self(waveform.to(self.device)) #.cuda())
...
class ResponseCLAPAudioEmbeddingClassifierFreev2(nn.Module):
    def __init__(
        self,
        pretrained_path,
        sampling_rate=16000,
        ...
        max_random_mute_portion=0.5,
        training_mode=True,
        pretrained_for_Only_text = None,
    ):
        super().__init__()
        self.device = "cuda"
        self.precision = "fp32"
        self.amodel = "HTSAT-response"  # or 'PANN-14'
        self.tmodel = "HTSAT-base"  # the best text encoder in our training
        self.enable_fusion = False  # False if you do not want to use the fusion model
        self.fusion_type = "aff_2d"
        self.pretrained = pretrained_path
        self.embed_mode = embed_mode
        ...
        # to process text when nessesary we initilise normal CLAP too
        self.model_for_Only_text = CLAPAudioEmbeddingClassifierFreev2(
                                                                       pretrained_for_Only_text,
                                                                        sampling_rate=16000,
                                                                        embed_mode="text",
                                                                        unconditional_prob=0.1,
                                                                        random_mute=False,
        ...
        for p in self.model_for_Only_text.parameters():
            p.requires_grad = False

        self.model_for_Only_text.eval()

    def get_unconditional_condition(self, batchsize):
        self.unconditional_token = self.model_for_Only_text.model.get_text_embedding(
            self.tokenizer(["", ""])
        )[0:1]
        return torch.cat([self.unconditional_token.unsqueeze(0)] * batchsize, dim=0)
        ...
        if self.embed_mode == "audio":
            ...
            with torch.no_grad():
                ...
                for waveform in self.batch_to_list(batch):
                    ...
                    audio_dict_list.append(audio_dict)
                # Assuming your model has a method get_audio_embedding that processes a list of audio dicts
                embed = self.model.get_text_embedding(audio_dict_list)
        embed = embed.unsqueeze(1)
        self.unconditional_token = self.model_for_Only_text.model.get_text_embedding(
            self.tokenizer(["", ""])
        )[0:1]
